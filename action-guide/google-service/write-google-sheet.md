...
* Credential file: Select the Google account authentication file (the `.json` file). This is a security certificate that authorizes GPM Automate to connect, authenticate, and execute the command to write content to your Google Sheet.
  * _Instructions to create the file_: Watch the tutorial video here:&#x20;
* File ID: The unique identifier of the Google Sheets file to interact with (taken from the string located between `/d/` and `/edit` in the file's URL).
* Sheet ID: The index number of the sheet to write content (starting from `0` for the first sheet from left to right).
* Column Name or Index: Specify the column position to write data. You can enter a letter (e.g., `A`, `B`, `C`...) or enter an index number (e.g., `1` corresponds to column A).
* Row Index: The index number of the row to write data, counted starting from `1`.
* Value: The text, number, or value from a GPM Automate variable that you want to fill into the target cell.
