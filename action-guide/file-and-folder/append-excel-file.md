...
Append excel file is an intelligent action used to automatically find the first empty row at the end of the data table and append content to it, allowing you not to manually calculate or manage the row index (`Row Index`).
...
Suppose you have an Excel file containing a current data list with entries from row 1 to row 5. You want to configure a script to automatically add new data (for example, a coupon code just scraped) into the next empty cell of column B (which is cell `B6`).
