...
Get clipboard text is an action used to read (retrieve) all the text content currently stored in the clipboard of the computer – specifically, the text you just copied (`Ctrl + C`) from any source – and then save that value into an output variable.
...
Suppose you are running an automated ordering script in single-thread mode. Before you click to run the thread, you browse the web and find a great discount code, you highlight and press `Ctrl + C` the string `GIAMGA50K` to save it to your computer's clipboard.
