...
Key press is an action that simulates keyboard operations, allowing you to type text, press single keys, or activate keyboard shortcuts on the browser interface.
...
* Xpath: The identifier path (XPath) of the input field (input, textarea) that you want to interact with.
...
* Delay each character: The time interval between each character when typing text. There are special configurations as follows:
  ...
  * _Mandatory note_: If your text contains emoji characters, you must configure this item as `-1` to avoid display errors.
