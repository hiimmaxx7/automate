...
Suppose you are running an automated ordering script in single-thread mode. Before you click to run the thread, you browse the web and find a great discount code, you highlight the text and press `Ctrl + C` to save the string `GIAMGA50K` to your computer's clipboard.
...
* Step 2: Call the input action (for example, Key press), passing the variable `$copiedText` into the discount code input field on the website.

Result: The system will automatically retrieve the string `GIAMGA50K` from the Windows clipboard and fill it directly into the website to apply the discount code, helping you smoothly combine manual actions and automated running threads.
