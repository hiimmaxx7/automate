...
Copy file is an action used to copy a file from one location to another on the computer's hard drive. This action will create an entirely new copy at the destination while keeping the original file intact at its initial location.
...
* Result: The system will take the file `template.xlsx`, create an exact copy, and name it `data_output.xlsx` in the specified directory. Subsequent actions of the script (such as _Write excel file_) will only interact with and write data to this `data_output.xlsx` file, keeping your sample file safe and clean for future runs.
