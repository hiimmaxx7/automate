...
* Purpose: Extremely useful when you need to send images through network commands like `HTTP Request`. For example, you want to send an image to AI services to decode a captcha, or upload an image to a server without transferring the file in the usual way.
...
When you navigate to a website and encounter an image-based Captcha. You have used the Screenshot action to capture the area containing the Captcha code and saved it as an image file at `D:\GPM_Data\captcha.png`. Now, you want to send this image to a third-party API (like 2Captcha, Anti-Captcha) to have them solve it for you. Since their API requires the image data to be in the form of a Base64 text string, you will configure it as follows:
...
Result: The image file `captcha.png` will be converted into a long string of code in the form `data:image/png;base64,iVBORw0KGgoAAAANSU...` stored in the variable `$base64String`. The subsequent HTTP Request command will simply send this string neatly, helping the automated script easily bypass Captcha security layers.
