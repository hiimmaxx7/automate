...
Count is an action used to count the number of elements in a one-dimensional array (or a data list) and return a numerical value. This action is very useful when you need to check the size of the data before processing or set limits for loops.
...
* How to do it: Use the action File read all lines to read all the lines in the file into a variable list, then use the action Count to count that list. The returned value will be the total number of lines in the file.
...
After running, the variable `$totalFiles` will receive the value of `3`. You can use this result to run conditional command blocks (for example: If the folder has more than 0 files, then proceed to upload images to Facebook/Website).
...
#### 3. Count the number of elements after splitting text

When you have a long text string containing multiple pieces of information separated by special characters and want to know how many elements will result after splitting:

* How to do it: After using the action Split text to split this string based on the character `|`, you let the action Count process the split list. The returned result of the count variable at this point will be `6`.

#### Practical example: Count the number of elements in the string after using Split text

Suppose you have a configuration file or a line of data containing account information formatted with the structure separated by the vertical bar `|` as follows:
