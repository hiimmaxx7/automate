...
To create an intelligent automation script that smoothly handles real-world scenarios on the website or software interface, combining this trio of actions in a tight logical flow is the optimal solution.

Below is a detailed explanation of the common technical parameters and the steps to set up the workflow for a practical scenario.

🎥 Watch the tutorial video: [Here](https://youtu.be/xWu0g35YTGo).

#### Core configuration parameters to master:

* Image: The target image area you cut from the screen to use as matching data for the system.
* Timeout: The maximum time limit for the system to search for the image (only applies to _Wait to image_).
* Threshold: The color deviation / tolerance rate when matching pixels (usually set from `0.7` to `0.95`). The closer the number is to `1`, the more the actual image must resemble the sample image absolutely. You can adjust this value and test until it fits.
...
**Step 1: Synchronize the page load flow with the Wait to image action**
...
* Operational logic: The script will pause to observe the screen. As soon as the registration form is displayed, the system immediately activates the next step without wasting time waiting.

<figure><img src="../../.gitbook/assets/is-01b.png" alt=""><figcaption></figcaption></figure>

**🔹 Step 2: Check the existence of the button with the Image exists action**
...
To ensure the script runs intelligently and does not miss clicks, we wrap the interaction steps inside the conditional block.
...
Inside the If block, when the condition is met, the system begins tracing to get the coordinates of the button (since the position of the registration form may shift flexibly depending on the screen resolution of each Profile).
...
**🔹 Step 5: Perform a mouse click with the Mouse click action**
