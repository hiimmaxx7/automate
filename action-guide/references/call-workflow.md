...
When the parent Workflow reaches the Call Workflow step, the system will pause the main flow and transfer execution rights to the child Workflow. After the child Workflow is completed, the system will automatically return to the parent Workflow and continue executing the next steps.
...
Suppose you are scripting to manage multiple accounts for MMO projects, instead of writing login code in every script, you create a separate Workflow named `login_module`:
...
• Step 4: When the main Workflow reaches this point, it will "call" `login_module` to complete the user/pass entry, click login, and handle captcha. After that, it automatically returns to the main Workflow to continue tasks such as running Tools, doing Airdrop, or posting products.

Technical notes:

• Ensure the child Workflow has been saved and its logic checked before calling.

• Check the consistency of variable names between the parent and child Workflows to ensure data is transmitted accurately and without conflict. To use a variable from the child workflow, you write in the syntax: `$workflow_name_variable_name`, for example, in the Main workflow, if the module login_module has a variable `$status`, then to use it in the Main workflow, you need to enter `$login_module_status`.
