...
* Purpose: For example, you can get the image path from the `src` attribute, or get the link from the `href` attribute of a link tag, or get the input value in the `value` field.
* Simple understanding: Every component on the web has accompanying "attributes" (like an information sticker). This command helps you read and "copy" the information from that sticker to save it.
