...
Suppose you are running a web scraping script and the system has extracted an online video link stored in the variable `$videoUrl`. You want to download this video to a fixed folder on drive D in preparation for the next processing steps:
...
Result: The system will automatically connect to the network, download the video file from the link `$videoUrl`, and output it as a physical file named `downloaded_video.mp4` neatly located in the `D:\GPM_Data\Videos\` folder. The variable `$downloadStatus` will receive a value so you can use an If block to check whether the file has been downloaded successfully before proceeding to the next actions.
