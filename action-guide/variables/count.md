...
Count is an action used to count the number of elements in a one-dimensional array (or a data list) and return a numeric value. This action is very useful when you need to check the size of data before processing or set limits for loops.
...
* How to do it: Use the action File read all lines to read all lines in the file into a variable list, then use the action Count to count that list. The returned value is the total number of lines in the file.
...
After running, the variable `$lineCount` will receive the value of `9`. You can immediately use this `$lineCount` variable as the End parameter for the For loop to automatically open each Facebook profile for interaction.
...
* Step 2: Use the action Count passing in the array `$fileList` just obtained, and name the output variable as `$totalFiles`.

After running, the variable `$totalFiles` will receive the value of `3`. You can use this result to run conditional command blocks (for example: If the folder has more than 0 files, then perform the action to upload images to Facebook/Website).
...
When you have a long text string containing multiple pieces of information separated by special characters and want to know how many elements will be after splitting:
...
#### Practical example: Count the number of elements of a string after using Split text

Suppose you have a configuration file or a line of data containing account information formatted in a structure separated by the vertical bar `|` as follows:

```
thanhnguyen|Pass1234|2FAXYZ|103.83.77.15:9898
```

To count how many pieces of information this string has for the purpose of checking the validity of the account, the script will be configured in 2 steps:
...
After running, the variable `$infoCount` will receive the value of `4`. You can use this result in conjunction with the If command block to check (for example: If `$infoCount = 4` meaning the account has sufficient structural information, then proceed to run the login script, otherwise skip to avoid errors).
