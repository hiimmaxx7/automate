...
Get URL is an action used to extract the entire address (URL) of the currently opened webpage in the browser and save it into a variable.

🎥 Watch the tutorial video: [Here](https://youtu.be/a9qNt96X1SM).

#### Configuration parameters:

* Output variable: The name of the variable you assign to store the webpage address string after successful extraction.

#### Practical example: Check the redirect status after successful login
...
* Configuration method:
  ...
  * Combine with an If conditional block afterwards to check the data of the variable `$currentLink`:
    ...
    * _Case 2_: If the variable `$currentLink` contains the phrase `home` or the data switches to the personal page ➔ The script identifies a successful login and continues to run the account nurturing tasks.